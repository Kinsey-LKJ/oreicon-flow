...
async function generateIconCode({ name }) {
  ...
  const svgCode = await processSvg(code.toString());
  ...
  fs.writeFileSync(destination, await component, 'utf-8');
...
(async () => {
  const iconPromises = Object.keys(icons)
    .map((key) => icons[key])
    .map(async ({ name }) => {
      const { ComponentName, name: iconName } = await generateIconCode({
        name,
      });
      appendToIconsIndex({ ComponentName, name: iconName });
    });
